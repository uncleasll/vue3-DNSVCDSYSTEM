...
const props = withDefaults(defineProps<{ targetPanelIds?: number[] }>(), { targetPanelIds: () => [] })
    ...
    <div class="relative min-h-0 flex-1 overflow-hidden rounded-xl border border-slate-200 bg-slate-50">
      <svg :viewBox="`0 0 ${width} ${height}`" class="h-full w-full" preserveAspectRatio="xMidYMid meet">
        <defs>
          ...
          <radialGradient id="camera-glow" cx="50%" cy="50%" r="50%"><stop offset="0%" stop-color="#2563eb" stop-opacity="0.28" /><stop offset="100%" stop-color="#2563eb" stop-opacity="0" /></radialGradient>
        ...
        <g :transform="`translate(${width / 2}, ${height / 2})`">
          ...
          <circle r="52" fill="#2563eb" stroke="#fff" stroke-width="10" class="animate-pulse" />
